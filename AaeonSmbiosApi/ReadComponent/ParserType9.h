...
public:
	SmbiosTable Parse(void* p, nlohmann::ordered_json& json_object)
	{
		...
		json_object["Table_9"]["name"] = "System Slots";
		UpdateJsonObject(json_object, pBIOS->Type, pBIOS->Handle, (UCHAR*)p);
		//If peer_grouping_count does not exist
		if (!json_object["Table_9"][json_handle].contains("peer_grouping_count"))
			return smbios_table;
		else
		{
			string s_peer_grouping_count = json_object["Table_9"][json_handle]["peer_grouping_count"]["value"].get<std::string>();
			unsigned int n = 0;
